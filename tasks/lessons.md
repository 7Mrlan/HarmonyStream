...
## 2026-05-27 - 产品路线纠偏必须写进 Spec，不要只留在聊天里

**触发**：用户指出我已经在对话里确认 Claudio 当前偏离“个人 AI 音乐电台”核心，但 `tasks/spec.md` 仍写着下一步默认做 Phase K 歌曲像素海报；新的 Codex / 新窗口没有聊天上下文，会继续按旧文档推进。

**根因**：我把关键产品判断停留在聊天回答，没有同步更新决策真源。对于多 agent / 多窗口协作，聊天不是可继承的项目状态；`tasks/spec.md` 和 `tasks/todo.md` 才是后续执行依据。

**规则**：1. 任何会改变产品定位、阶段优先级、HARD-GATE 或“不做什么”的结论，必须当轮写入 `tasks/spec.md`。2. 如果结论影响下一步执行，还必须同步 `tasks/todo.md` 当前主线和当前任务。3. 长对话里的路线纠偏不能只总结给用户看，要固化为新窗口可直接继承的文档。4. 写完文档后用 `rg` 反查旧优先级关键词，避免旧路线残留继续误导后续 agent。

**关联文件**：`tasks/spec.md`、`tasks/todo.md`、`tasks/lessons.md`
