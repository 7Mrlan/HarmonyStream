 ...
 * 用户音乐资料解析
 * ----------------
 * 这里只处理文本 / JSON / JSONL 到内部资料结构的转换，不做文件 IO。
 * profileStore 负责读写入口，解析层保持纯函数，方便后续扩展资料模板。
 */
...
export interface UserMusicProfileFileTexts {
  /* taste.md 原文。 */
  taste: string | null;
  /* routines.md 原文。 */
  ...
  /* simple-playlists.json 原文。 */
  simplePlaylists: string | null;
  /* qq_songs.json 原文。 */
  qqSongs: string | null;
  /* library-insights.json 原文。 */
  libraryInsights: string | null;
  /* listening-events.jsonl 原文。 */
  listeningEvents: string | null;
  /* dj-memory.json 原文。 */
  djMemory: string | null;
  /* mood-rules.md 原文。 */
  moodRules: string | null;
}
...
/*
 * 从已读取的资料文本构造完整 UserMusicProfile。
 * 文件缺失和 JSON 错误都会被收敛为空分支，避免用户资料拖垮 /api/chat。
 */
export function buildUserMusicProfileFromFiles(files: UserMusicProfileFileTexts): UserMusicProfile {
  const librarySections = [
    ...parseSimplePlaylistSections(files.simplePlaylists, 'simple-playlists.json'),
    ...parseSimplePlaylistSections(files.qqSongs, 'qq_songs.json'),
  ];
  const legacyTracks = parsePlaylistTracks(files.playlists);
  const simpleTracks = librarySections.flatMap(sectionToTrackSeeds);
  const loadedFiles = [
    files.taste ? 'taste.md' : '',
    files.routines ? 'routines.md' : '',
    files.playlists ? 'playlists.json' : '',
    files.simplePlaylists ? 'simple-playlists.json' : '',
    files.qqSongs ? 'qq_songs.json' : '',
    files.libraryInsights ? 'library-insights.json' : '',
    files.listeningEvents ? 'listening-events.jsonl' : '',
    files.djMemory ? 'dj-memory.json' : '',
    files.moodRules ? 'mood-rules.md' : '',
  ...
  return {
    source,
    loadedFiles,
    tasteSummary: normalizeProfileText(files.taste ?? '') ?? '',
    tracks: dedupeTracks([...simpleTracks, ...legacyTracks]),
    librarySections,
    libraryInsights: parseInsightRecords(files.libraryInsights, 'insight'),
    listeningEvents: parseListeningEvents(files.listeningEvents),
    djMemory: parseInsightRecords(files.djMemory, 'memory'),
    routines: parseRoutineRules(files.routines),
    moodRules: parseMoodRules(files.moodRules),
  };
}
...
function collectRawTracks(parsed: unknown): RawPlaylistTrack[] {
  ...
  const playlistTracks = Array.isArray(file.playlists)
    ? file.playlists.flatMap((item) => {
        ...
        const source = normalizeProfileText(item.name);
        const tracks = Array.isArray(item.tracks) ? item.tracks.filter(isRecord) : [];
        return tracks.map((track) => ({ ...track, source: normalizeProfileText(track.source) ?? source }));
      })
    : [];
...
function normalizeTrackSeed(value: RawPlaylistTrack): PersonalTrackSeed | null {
  const title = normalizeProfileText(value.title) ?? normalizeProfileText(value.name);
  if (!title) return null;

  const artist = normalizeProfileText(value.artist);
  const note = normalizeProfileText(value.note);
  const source = normalizeProfileText(value.source);
...
function normalizeInsightRecord(
  ...
  const summary =
    normalizeProfileText(value.summary) ?? normalizeProfileText(value.text) ?? normalizeProfileText(value.note);
  if (!summary) return null;

  const id = normalizeProfileText(value.id) ?? `${kind}-${index + 1}`;
  return {
    id,
    summary,
...
function normalizeListeningEvent(value: RawListeningEventRecord, index: number): ListeningEvent | null {
  ...
  const title = normalizeProfileText(value.title) ?? normalizeProfileText(value.name);
  const artist = normalizeProfileText(value.artist);
  const text = normalizeProfileText(value.text) ?? normalizeProfileText(value.note);
  const at = normalizeProfileText(value.at) ?? normalizeProfileText(value.time) ?? normalizeProfileText(value.timestamp);

  return {
    id: normalizeProfileText(value.id) ?? `event-${index + 1}`,
    type,
    ...(title ? { title } : {}),
    ...(artist ? { artist } : {}),
    ...(text ? { text } : {}),
...
function normalizeListeningEventType(value: unknown): ListeningEventType | null {
  const text = normalizeProfileText(value)?.toLowerCase();
  if (!text) return null;
...
function normalizeLibrarySection(
  ...
  const name = normalizeProfileText(value.name) ?? normalizeProfileText(value.title) ?? `导入歌单 ${index + 1}`;
  const description = normalizeProfileText(value.description);
...
/* 归一化文本字段。 */
export function normalizeProfileText(value: unknown): string | undefined {
